...
import { CheckCircle2, AlertCircle, XCircle, Film, Calendar, Clock, Monitor, Armchair } from 'lucide-react'

type ScanState =
  | { type: 'idle' }
  | { type: 'success'; ticket: Ticket; showTitle: string; screenName: string; showTime: string }
  | { type: 'duplicate'; message: string; checkInTime?: string }
...
export default function EntryScanner() {
      ...
      try {
        ...
        if (t.status === 'checked-in') {
          await supabase.from('scan_logs').insert({
            ...
            message: `Duplicate check-in attempt — already checked in at ${t.check_in_time ? formatDateTime(t.check_in_time) : 'unknown time'}`,
          })
          triggerScanResult({
            type: 'duplicate',
            message: 'Already checked in.',
            checkInTime: t.check_in_time ?? undefined,
          })
          return
        }

        if (t.status === 'checked-out' || t.status === 'no-show') {
          const msg = t.status === 'checked-out' ? 'Ticket already checked out' : 'Ticket marked as no-show'
          await supabase.from('scan_logs').insert({
            ticket_code: cleanCode,
            show_id: t.show.id,
            event_type: 'invalid',
          ...
          triggerScanResult({ type: 'error', message: msg + '.' })
          return
        }
        ...
        await supabase.from('scan_logs').insert({
          ...
          show_id: t.show.id,
          event_type: 'checkin',
          message: `Checked in: ${t.customer_name}`,
        })
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <h2 className="text-2xl font-bold text-brass-50">Entry Scanner</h2>
        <p className="text-ink-400 text-sm mt-1">
          Scan any ticket QR code — the show is detected automatically
        </p>
      </div>

      <ScannerPanel onScan={handleScan} title="Entry Scanner" accentColor="teal" />
      ...
      <div className="scan-result scan-success fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[calc(100%-2rem)] max-w-2xl">
        <div className="bg-teal-500/15 border-2 border-teal-500 rounded-2xl px-6 py-5 shadow-2xl shadow-teal-500/20 backdrop-blur-sm">
          <div className="flex items-center gap-4 mb-3">
            ...
            <div>
              <p className="text-xl font-display font-bold text-teal-400">Checked In</p>
      ...
      <div className="scan-result scan-error fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[calc(100%-2rem)] max-w-2xl">
        <div className="bg-red-500/15 border-2 border-red-500 rounded-2xl px-6 py-5 flex items-center gap-4 shadow-2xl shadow-red-500/20 backdrop-blur-sm">
          ...
          <div className="flex-1">
            <p className="text-xl font-display font-bold text-red-400">Duplicate Check-In</p>
            <p className="text-brass-50 text-sm mt-0.5">{state.message}</p>
            {state.checkInTime && (
              <p className="text-xs text-ink-400 mt-1">
                Original check-in: {formatDateTime(state.checkInTime)}
              </p>
            )}
          </div>
        </div>
